...
class Pergunta5SolidaoScreen extends StatefulWidget {
  const Pergunta5SolidaoScreen({super.key});

  @override
  State<Pergunta5SolidaoScreen> createState() => _Pergunta5SolidaoScreenState();
}

class _Pergunta5SolidaoScreenState extends State<Pergunta5SolidaoScreen> {
  ...
  Future<void> _checkForLoop() async {
    final redirectRoute = await _loopDetector.checkForLoop('SolidaoP5', context);
    if (redirectRoute != null && mounted) {
      Navigator.pushReplacementNamed(context, redirectRoute);
    } else {
      _loopDetector.registerVisit('SolidaoP5');
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    if (_isChecking) {
      return const Scaffold(
        backgroundColor: Color(0xFFFFE8F3),
        body: Center(
